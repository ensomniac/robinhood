...
Status: selection implementation is tested but not yet frozen; no provider or
target-outcome access is authorized
...
`development_tranche.py` freezes the next exact 100 target sessions before any
new reference or market collection. Its outcome-blind preflight currently finds
...
seeded selection needs 243 target/lookback sessions. These counts are preflight
facts, not a frozen selection or an alpha result.
