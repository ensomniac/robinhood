"""Inspect v13 development data and bind it to the frozen search."""
...
import earnings_sec_reaction_v13_collection as source
...
class EarningsSecReactionV13CollectionInspectionError(RuntimeError):
    """The exact v13 development dataset failed validation."""
...
def inspect_collection(
    ...
    *,
    ...
    if (
        ...
        raise EarningsSecReactionV13CollectionInspectionError(
            "private v13 dataset is missing or drifted"
        )
    try:
        dataset = json.loads(gzip.decompress(private_path.read_bytes()))
    except (OSError, json.JSONDecodeError) as exc:
        raise EarningsSecReactionV13CollectionInspectionError(
            "private v13 dataset is unreadable"
        ) from exc
    ...
    if not all(checks.values()):
        raise EarningsSecReactionV13CollectionInspectionError(
            "v13 development collection inspection failed"
        )
    ...
    value: dict[str, Any] = {
        "schema_version": 1,
        "artifact_kind": (
            "earnings-sec-reaction-v13-collection-inspection"
        ),
        "campaign_id": source.CAMPAIGN_ID,
        "family_id": source.FAMILY_ID,
        "successor_id": source.SUCCESSOR_ID,
        "state": "REACTION_V13_DEVELOPMENT_DATASET_INSPECTED_READY",
