"""Discovery plugin for S&P 500 deletion forced-selling rebound."""
...
class Sp500DeletionPluginError(RuntimeError):
    """Historical or production deletion semantics drifted."""
...
def _parameters(value: Mapping[str, Any]) -> dict[str, Any]:
    normalized = {
        ...
        "maximum_positive_effective_gap_fraction": float(
            value["maximum_positive_effective_gap_fraction"]
        ),
        "minimum_pre_effective_decline_fraction": float(
            value["minimum_pre_effective_decline_fraction"]
        ),
        "minimum_sessions_to_effective": int(
            value["minimum_sessions_to_effective"]
        ),
        "stop_buffer_below_pre_effective_close": float(
            value["stop_buffer_below_pre_effective_close"]
        ),
    }
    if (
        normalized["maximum_hold_sessions"] not in {2, 5}
        or normalized["maximum_positive_effective_gap_fraction"]
        not in {0.02, 0.04}
        or normalized["minimum_pre_effective_decline_fraction"]
        not in {0.02, 0.05}
        or normalized["minimum_sessions_to_effective"] not in {2, 4}
        or normalized["stop_buffer_below_pre_effective_close"]
        not in {0.0, 0.01}
    ):
        raise Sp500DeletionPluginError(
...
def _events(
    value: Any, decision_date: str, next_session: str
) -> list[dict[str, Any]]:
    if not isinstance(value, list) or not value:
        raise Sp500DeletionPluginError(
            "live official deletion set is empty"
        )
    required = {
        "action",
        "announcement_at",
    ...
    for raw in value:
        if not isinstance(raw, Mapping) or set(raw) != required:
            raise Sp500DeletionPluginError(
                "live official deletion schema drifted"
            )
        row = dict(raw)
        event_id = row.pop("event_id")
        if (
            event_id != canonical_sha256(row)
            or row["action"] != "Deletion"
            or row["index_name"] != "S&P 500"
            or row["pre_effective_date"] != decision_date
            ...
            raise Sp500DeletionPluginError(
                "live official deletion identity or timing drifted"
            )
    ...
    if rows != static_order:
        raise Sp500DeletionPluginError(
            "live official deletion metadata ranks drifted"
        )
    return rows


def _positive_map(
    value: Any, symbols: set[str], field: str
) -> dict[str, float]:
    if not isinstance(value, Mapping) or set(value) != symbols:
        raise Sp500DeletionPluginError(
            f"live {field} are incomplete"
        )
    try:
        parsed = {symbol: float(value[symbol]) for symbol in symbols}
    except (TypeError, ValueError) as exc:
        raise Sp500DeletionPluginError(
            f"live {field} are invalid"
        ) from exc
    if any(item <= 0 for item in parsed.values()):
        raise Sp500DeletionPluginError(
            f"live {field} must be positive"
        )
    return parsed
...
def evaluate_production(
    ...
    """Select one effective-open rebound through historical semantics."""
    ...
    references = _positive_map(
        decision["reference_closes"], symbols, "reference closes"
    )
    pre_effective = _positive_map(
        decision["pre_effective_closes"],
        symbols,
        "pre-effective closes",
    )
    ranked = sorted(
        events,
        key=lambda row: (
            -(1 - pre_effective[str(row["ticker"])]
              / references[str(row["ticker"])]),
            -int(row["sessions_to_effective"]),
            str(row["ticker"]),
            str(row["event_id"]),
        ),
    ...
    if not isinstance(quotes, list) or not quotes:
        raise Sp500DeletionPluginError("live quotes are missing")
    ...
    for raw in quotes:
        ...
        if (
            ...
            raise Sp500DeletionPluginError(
                "live quote is not a valid effective-open observation"
            )
        by_symbol[symbol] = quote
    if set(by_symbol) != symbols:
        raise Sp500DeletionPluginError(
            "live quotes do not cover every ranked deletion"
        )
    selected: tuple[
        dict[str, Any],
        dict[str, Any],
    ...
    for event in ranked:
        ...
        ask = float(quote["ask"])
        decline = 1 - pre_effective[symbol] / references[symbol]
        entry_gap = ask / pre_effective[symbol] - 1
        expected_gross = decline - max(entry_gap, 0.0)
        stop = pre_effective[symbol] * (
            1 - parameters["stop_buffer_below_pre_effective_close"]
        )
        if (
    ...
    if selected is None:
        raise Sp500DeletionPluginError(
            "no exact live S&P deletion rebound signal"
        )
    event, quote, stop, decline, entry_gap, expected_gross = selected
