"""Independently inspect second-tranche pre-entry contracts and triggers."""
...
class ChallengerPreentryInspection2Error(RuntimeError):
    """The independently reconstructed second-tranche evidence differs."""
...
def inspect_contract(*, manifest_path: Path, env_path: Path) -> dict:
    ...
    if not (
        ...
        and rebuilt["pair_identity_sha256"]
        == contract.get("pair_identity_sha256")
        and rebuilt["request_graph_sha256"]
        ...
        raise ChallengerPreentryInspection2Error(
            "second-tranche pre-entry selection does not rebuild"
        )
    for value in manifest.get("implementation_contract", {}).values():
        ...
        if preentry._sha256_file(path) != value.get("sha256"):
            raise ChallengerPreentryInspection2Error(
                "second-tranche implementation binding differs"
            )
    ...
    if artifacts != {
        ...
        raise ChallengerPreentryInspection2Error(
            "second-tranche pre-entry zero state differs"
        )
    if shutil.disk_usage(config.root).free < config.min_free_bytes:
        raise ChallengerPreentryInspection2Error("historical reserve is unavailable")
    return {
        "schema_version": 1,
...
def inspect_result(
    *, manifest_path: Path, env_path: Path, result_path: Path
    ...
    if rebuilt != recorded:
        raise ChallengerPreentryInspection2Error(
            "second-tranche trigger result differs"
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        ChallengerPreentryInspection2Error,
