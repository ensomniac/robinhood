"""Freeze and run the second-tranche trigger through its exact-window reader."""
...
import challenger_orb_retest_preentry2 as preentry2
...
SOURCE_MANIFEST = (
    ...
    / (
        ...
        "tranche2-v1-"
        "5a6ac82d0c1205ae915e91d629d921b309d4862525b979ade877dbc3f7ec9108.json"
    )
)
...
INSPECTOR = PROJECT_ROOT / "challenger_orb_retest_trigger_runner_inspection2.py"
DEFAULT_OUTPUT_ROOT = (
    PROJECT_ROOT
    / "historical_batches/challenger_orb_retest_v1_tranche2/trigger_manifests"
)
DEFAULT_STATUS = (
    PROJECT_ROOT
    / "historical_batches/challenger_orb_retest_v1_tranche2/trigger-status.json"
)
EXPECTED_REQUESTS = 336
EXPECTED_ROWS = 4_325_297
EXPECTED_PAIRS = 113
EXPECTED_DATES = 55


class ChallengerTriggerRunner2Error(RuntimeError):
    """The second-tranche runner adapter or frozen base has drifted."""
...
def _source_state(
    ...
    if not (
        ...
        raise ChallengerTriggerRunner2Error(
            "inspected second-tranche causal collection differs"
        )
    return manifest, config, selection


def configure_base() -> None:
    """Install the second-tranche contract into the hash-pinned runner."""

    if _sha256_file(BASE_PATH) != BASE_IMPLEMENTATION_SHA256:
        raise ChallengerTriggerRunner2Error(
            "frozen base trigger runner implementation drifted"
        )
    preentry2.configure_base()
    values: dict[str, Any] = {
        "preentry": preentry2.base,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "freeze":
            ...
            value = preentry2.base._read_json(args.status)
        elif args.manifest is None:
            raise ChallengerTriggerRunner2Error("--manifest is required")
        else:
            value = base.derive(
        ...
        ChallengerTriggerRunner2Error,
        base.ChallengerTriggerRunnerError,
        preentry2.ChallengerPreentry2Error,
        preentry2.base.ChallengerPreentryError,
