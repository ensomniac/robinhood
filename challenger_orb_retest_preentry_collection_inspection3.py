"""Independently inspect third-tranche causal pre-entry transport."""
...
class ChallengerPreentryCollectionInspection3Error(RuntimeError):
    """The third-tranche causal collection does not reconcile."""
...
def inspect_collection(
    *, manifest_path: Path, env_path: Path, status_path: Path, result_path: Path
    ...
    if not (
        ...
        raise ChallengerPreentryCollectionInspection3Error(
            "third-tranche collection boundary differs"
        )
    result["inspection_implementation"].update(
        {
            "adapter_inspector_sha256": challenger.base._sha256_file(Path(__file__)),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        ChallengerPreentryCollectionInspection3Error,
