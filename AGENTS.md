...
Inspection `886230a1...cb7948` rebuilt the contract, controller hash, complete
...
independently rebuilt by inspection `71814897...6efcb2`. All ten inspection
checks pass across all 312 cached official pages, 270 tradable ticker-events,
184 announcement dates, publication timestamps, effective dates, ticker
identities, deduplication, zero provider requests, and zero market-price access.
...
Dataset-bound preflight `5e34336b...5abfa7` revalidates all 633 frozen events,
current implementation hashes, and exact manifest binding as `CAPACITY_READY`
without opening outcomes. Commit and push it before final search freeze.
...
in zero provider requests. Search `dd5e80f1...bfa5e` now binds all 32 trials,
...
an existing-family replication and consumes no new-mechanism slot. Contract
...
to search `632229c0...268f92`. Development result
`27b9c7cf...532319` evaluated the sole trial with 70 fills and zero provider
...
development inspection rejected all 32 frozen trials. Eight trials retained
positive 20-bps growth and seven retained stressed profit factor, but none
passed the frozen Deflated Sharpe or Holm family gates and only two had positive
growth in every rolling fold. Confirmation remained untouched. Its v2
predecessor failed
...
or symbols. The immediate data-ready successor is
`macro-etf-trend-pullback-v4-outcome-clean-replication`. It deterministically
uses the same inspected 2016-2022 calendar partition and unchanged 32-rule
pullback grid, but only the six authorized macro ETFs (`DBC`, `EEM`, `EFA`,
...
remain untouched. Independent inspection rejected all 32 trials: only two had
positive 20-bps growth and acceptable drawdown, while none passed stressed
profit factor, DSR, Holm, every rolling fold, or neighbor stability. The best
