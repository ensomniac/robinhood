"""Freeze a country-equity ETF closing-strength continuation family."""
...
MECHANISM_FAMILY = "closing-auction-pressure-continuation"
STRATEGY_ID = "country-equity-etf-close-strength-continuation"
SUCCESSOR_ID = "country-equity-etf-close-strength-continuation-v1"
...
class EtfCloseStrengthContinuationError(ValueError):
    """The close-strength authority, scope, or exact rules drifted."""
...
def _repo_path(path: Path) -> str:
    try:
        ...
        raise EtfCloseStrengthContinuationError(
...
def freeze_contract(
    *, created_at: str, enforce_commit: bool = True
    ...
    contract: dict[str, Any] = {
        ...
        "mechanism": (
            "A cost-clearing completed advance that closes near the daily "
            "high can reveal persistent closing-auction demand whose price "
            "discovery continues after the next observable open."
        ),
        "expected_holding_behavior": (
            "Long only, next-session-open entry, and flat within two sessions."
        ),
        "entry_rule": (
            "Require completed close location at or above the frozen "
            "threshold, a completed one-session advance at least the frozen "
            "cost-clearing floor, and close above the frozen SMA; rank the "
            "highest close location and enter at the next session open."
        ),
        "stop_rule": (
        ...
        "ranking_rule": (
            "Highest completed close location, then largest one-session "
        ...
        "material_difference_rationale": (
            "The candidate tests continuation of closing-auction demand after "
            "positive completed returns. It is directionally and causally "
            "distinct from prior oversold, IBS reversal, residual reversal, "
            "gap, cross-asset refuge, and trend-pullback families."
        ),
        "parameter_grid": {
            "close_location_minimum": [0.8, 0.9],
            "maximum_hold_sessions": [1, 2],
            "one_session_advance_fraction": [0.005, 0.01],
            "stop_atr14": [1.0, 1.5],
            "trend_sma": [100, 200],
        },
        ...
        "universe_requirements": {
            ...
            "selection_basis": (
                "Nine fixed U.S.-listed country equity ETFs with pre-2008 "
        ...
        "contamination_risks": [
            ...
            "No prior adverse family selected a close-location continuation threshold.",
        ...
        "falsifiers": [
            ...
            "closing-strength continuation depends on a few crisis rebounds",
        ],
...
def validate_contract(
    ...
    if not (
        ...
        raise EtfCloseStrengthContinuationError(
            "close-strength family contract drifted"
        )
    if enforce_commit:
        strategy_discovery.require_committed(Path(__file__).resolve())
    ...
    if not (
        ...
        raise EtfCloseStrengthContinuationError(
            "close-strength calendar binding drifted"
        )
...
def main() -> int:
    ...
    try:
        ...
        EtfCloseStrengthContinuationError,
        OSError,
        ValueError,
