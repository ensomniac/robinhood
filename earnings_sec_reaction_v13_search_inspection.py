"""Independently inspect the frozen v13 SEC reaction search."""
...
import earnings_sec_reaction_v13_search as source
...
class EarningsSecReactionV13SearchInspectionError(RuntimeError):
    """The v13 pre-outcome search failed independent reconstruction."""
...
def inspect_search(
    ...
    *,
    ...
    checks = {
        ...
        "complete_cumulative_64_trials": contract.get(
            "selection_accounting", {}
        ).get("cumulative_trial_count")
        == 64
        and len(contract.get("prior_trial_sharpes", [])) == 32
        and len(contract.get("prior_trial_p_values", [])) == 32,
        "prior_paths_exact": contract.get(
            "prior_trial_daily_returns_by_id"
        )
        == prior["prior_trial_daily_returns_by_id"]
        and set(contract["prior_trial_daily_returns_by_id"])
        == trial_ids,
        "prior_statistics_exact": contract.get("prior_trial_sharpes")
        == prior["prior_trial_sharpes"]
        and contract.get("prior_trial_p_values")
        == prior["prior_trial_p_values"],
        "cumulative_pbo_method_exact": contract.get(
            "selection_accounting", {}
        ).get("cumulative_pbo_method")
        == prior["cumulative_pbo_method"]
        and contract.get("prior_pbo_probability") == 0.0,
        "complete_635_requests": len(request_rows)
        ...
        "capacity_metadata_only": capacity_manifest["dataset_payload"][
            "dense_capacity"
    ...
    if not all(checks.values()):
        raise EarningsSecReactionV13SearchInspectionError(
            "v13 frozen search inspection failed"
        )
    value: dict[str, Any] = {
        "schema_version": 1,
        "artifact_kind": (
            "earnings-sec-reaction-v13-search-inspection"
        ),
