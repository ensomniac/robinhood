"""Independently inspect the challenger's frozen accession-chain contract."""
...
class ChallengerSecAccessionChainInspectionError(RuntimeError):
    """The independently rebuilt challenger recovery contract differs."""
...
def inspect_contract(*, manifest_path: Path, env_path: Path, status_path: Path):
    ...
    if not (
        ...
        raise ChallengerSecAccessionChainInspectionError(
            "challenger recovery inspection boundary differs"
        )
    return result
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        ChallengerSecAccessionChainInspectionError,
