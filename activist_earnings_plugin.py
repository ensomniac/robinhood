"""Discovery plugin for activist-issuer SEC Item 2.02 continuation."""
...
from activist_earnings_discovery import EXPERIMENT_ID, FAMILY_ID
...
class ActivistEarningsPluginError(RuntimeError):
    """A frozen family input or exact production fact is incomplete."""


def _path(value: Any) -> Path:
    if not isinstance(value, str) or not value:
        raise ActivistEarningsPluginError("frozen path is missing")
    path = Path(value)
...
def _require_committed(path: Path) -> None:
    ...
    if tracked.returncode or clean.returncode:
        raise ActivistEarningsPluginError(
            f"activist earnings evidence must be committed: {relative}"
        )
...
def _manifest(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ActivistEarningsPluginError(
            f"activist earnings dataset manifest is invalid: {exc}"
        ) from exc
...
def preflight(contract: Mapping[str, Any]) -> dict[str, Any]:
    ...
    capacity = payload.get("activist_earnings_capacity")
    if not isinstance(capacity, Mapping):
        raise ActivistEarningsPluginError("capacity binding is missing")
    ...
    checks = {
        ...
        "confirmation_dates_bound": capacity.get(
            "confirmation_clean_entry_dates"
        )
        == contract.get("confirmation_signal_capacity"),
    }
    formal_capacity = int(capacity["development_event_pairs"]) + int(
        capacity["confirmation_clean_event_pairs"]
    )
    return {
...
def _load_private_dataset(binding: Mapping[str, Any]) -> dict[str, Any]:
    raw = str(binding.get("relative_path", ""))
    if (
        ...
        raise ActivistEarningsPluginError("private dataset binding is invalid")
    store = HistoricalDayStore.from_env()
    path = store.root / raw
    if not path.is_file():
        raise ActivistEarningsPluginError("private dataset is missing")
    ...
    if (
        ...
        raise ActivistEarningsPluginError("private dataset content drifted")
    return value
...
def _load_bound_dataset(
    ...
    *,
    ...
    manifest = _manifest(manifest_path)
    payload = manifest["dataset_payload"]
    binding = payload.get("activist_earnings_runtime")
    if not (
        ...
        raise ActivistEarningsPluginError("runtime dataset scope drifted")
    ...
    if (
        ...
        raise ActivistEarningsPluginError("private runtime dataset drifted")
    return dataset
...
def evaluate_production(
    ...
    required = {
        ...
        "activist_identity_point_in_time",
        "reaction_session_complete",
    ...
    passed = (
        ...
        and decision_data["activist_identity_point_in_time"] is True
        and decision_data["reaction_session_complete"] is True
