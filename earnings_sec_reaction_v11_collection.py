"""Collect the 108-symbol graph bound to the inspected v11 search."""
...
PRIVATE_NAMESPACE = "_derived/earnings_sec_reaction_v11"
...
class EarningsSecReactionV11CollectionError(RuntimeError):
    """The inspected v11 search or exact collection graph drifted."""
...
def _fetch(
    ...
    try:
        ...
        reason = PERMANENT_MISSING_ERRORS.get(str(exc))
        if reason is None:
            raise
        ...
        value["task_sha256"] = v5.self_hash(
            value, "task_sha256"
        )
        return value
...
def collect(
    ...
    *,
    ...
    if not (
        ...
        and inspection.get("authorized_provider_requests") == 108
    ):
        raise EarningsSecReactionV11CollectionError(
            "v11 search collection authorization is invalid"
        )
    ...
    if (
        ...
        raise EarningsSecReactionV11CollectionError(
            "v11 metadata selection drifted"
        )
    telemetry: dict[str, Any] = {
        "requests": 0,
        "request_seconds": 0.0,
        ...
        for index, request in enumerate(
            ...
            if path.is_file():
                ...
                if not (
                    ...
                    raise EarningsSecReactionV11CollectionError(
                        "v11 task does not match the frozen request"
                    )
    ...
    if (
        ...
        raise EarningsSecReactionV11CollectionError(
            "v11 request and cache accounting is incomplete"
        )
    ...
    for task in tasks:
        if task["status"] == "COMPLETE":
            ...
            missing[str(task["symbol"])] = str(
                task["missing_reason"]
            )
        else:
            raise EarningsSecReactionV11CollectionError(
                "v11 task status is invalid"
            )
    dataset = {
        "schema_version": 1,
        "family_id": FAMILY_ID,
    ...
    value: dict[str, Any] = {
        "schema_version": 1,
        "artifact_kind": (
            "earnings-sec-reaction-v11-development-collection"
        ),
        "campaign_id": CAMPAIGN_ID,
        "family_id": FAMILY_ID,
        "successor_id": SUCCESSOR_ID,
        "state": "REACTION_V11_DEVELOPMENT_COLLECTED_UNINSPECTED",
        "collected_at": metadata._timestamp(
