"""Freeze and evaluate the short-horizon oversold-reversal Stage 0 trial."""
...
LOOKBACK_BARS = 30
RSI_PERIOD = 5
MAX_RETURN_FRACTION = -0.03
MAX_RSI = 20.0
TARGET_R = 1.5


class OversoldReversalError(RuntimeError):
    """The oversold-reversal Stage 0 evidence is invalid."""
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise OversoldReversalError(str(exc)) from exc
...
def _variant() -> dict[str, Any]:
    ...
    if len(matches) != 1:
        raise OversoldReversalError("frozen slate variant identity is unavailable")
    ...
    manifest.update(
        {
            ...
            "selection_contract": {
                ...
                "lookback": "the 30 completed one-minute bars immediately preceding the trigger bar",
                "selloff_return": "last lookback close divided by first lookback close minus one is at most -0.03",
                "simple_rsi": "simple RSI over the last five completed close changes ending on the bar immediately before the trigger; flat RSI is 50",
                "oversold_gate": "pre-trigger simple RSI is at most 20",
                "session_vwap": "cumulative volume-weighted typical price through the completed trigger bar",
                "trigger": "bullish completed close strictly above both the prior bar high and session VWAP",
                "signal_bar_starts_et": "10:00:00-14:30:00 inclusive",
                "entry": "next observed one-minute open",
                "daily_selection": "earliest entry, then most negative selloff return, then lowest RSI, then lexical symbol",
                "miss_policy": "invalid bars or nonpositive stop or target distance remains in denominator and cannot signal",
            },
            "outcome_contract": {
                "stop": "lowest completed session low through the trigger bar",
                "target": "raw 1.5R above entry",
...
def _validate_manifest(recorded: Mapping[str, Any]) -> None:
    if recorded.get("manifest_sha256") != common._self_hash(
        ...
        raise OversoldReversalError("activation manifest content hash is invalid")
    if (
        ...
        raise OversoldReversalError("activation manifest identity is invalid")
    if recorded.get("implementation_sha256") != sha256_file(Path(__file__).resolve()):
        raise OversoldReversalError("activation implementation drifted")
    for field in (
        "development_evidence_eligible",
        "confirmation_evidence_eligible",
        "provider_requests_authorized",
        ...
        if recorded.get(field) is not False:
            raise OversoldReversalError(f"activation manifest {field} must be false")
...
def inspect_activation(
    ...
    if recorded != build_manifest(source):
        raise OversoldReversalError("activation manifest does not exactly rebuild")
...
def _validate_inspection(
    ...
    if recorded.get("inspection_sha256") != common._self_hash(
        ...
        raise OversoldReversalError("activation inspection content hash is invalid")
    if recorded != inspect_activation(manifest_path, store):
        raise OversoldReversalError("activation inspection does not exactly rebuild")
...
def _simple_rsi(
    rows: Sequence[Mapping[str, Any]], index: int, period: int = RSI_PERIOD
) -> float | None:
    if index < period:
        return None
    changes = [
        float(rows[offset]["close"]) - float(rows[offset - 1]["close"])
...
def _candidate(*, day: str, raw: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(bars, list):
        raise OversoldReversalError(f"{day} {symbol}: bars are malformed")
    ...
    for index in range(LOOKBACK_BARS, len(bars) - 1):
        ...
        if observed > SIGNAL_END:
            break
        lookback = bars[index - LOOKBACK_BARS : index]
        selloff_return = float(lookback[-1]["close"]) / float(lookback[0]["close"]) - 1
        if selloff_return > MAX_RETURN_FRACTION + 1e-12:
            continue
        rsi = _simple_rsi(bars, index - 1)
        if rsi is None or rsi > MAX_RSI + 1e-12:
            continue
        signal_close = float(bars[index]["close"])
        if (
            signal_close <= float(bars[index]["open"])
            or signal_close <= float(bars[index - 1]["high"])
            or signal_close <= vwap[index]
        ):
            continue
        entry_index = index + 1
        entry_open = float(bars[entry_index]["open"])
        stop = min(float(row["low"]) for row in bars[: index + 1])
        if entry_open <= stop:
            return {
                "date": day,
                "symbol": symbol,
        ...
        target = entry_open + TARGET_R * (entry_open - stop)
    ...
    return {"date": day, "symbol": symbol, "status": "no_oversold_reversal"}
...
def _trade_outcome(
    ...
    if exit_price is None:
        raise OversoldReversalError("trade did not resolve by force-flat")
    ...
    if planned_risk <= 0:
        raise OversoldReversalError("cost-adjusted planned risk is not positive")
    return {
...
def inspect_result(
    ...
    if recorded.get("result_sha256") != common._self_hash(recorded, "result_sha256"):
        raise OversoldReversalError("Stage 0 result content hash is invalid")
    ...
    if recorded != rebuilt:
        raise OversoldReversalError("Stage 0 result does not independently rebuild")
    audit: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
...
def _publish(value: Mapping[str, Any], path: Path) -> None:
    ...
    if not identity or not path.name.endswith(f"-{identity}.json"):
        raise OversoldReversalError("output filename must end with its content hash")
