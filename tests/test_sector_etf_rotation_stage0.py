...
def test_published_result_fails_profit_factor_drawdown_and_cost_stress():
    matches = sorted(
        (ROOT / "research_results").glob(
            "2026-07-21-sector-etf-rotation-stage0-*.json"
        )
    )
    assert len(matches) == 1
    ...
    assert result["denominator"]["decision_dates"] == 752
    assert result["denominator"]["closed_signals"] == 149
