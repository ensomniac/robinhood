...
import challenger_orb_retest_catalyst_contract3_v2_inspection as inspector
...
def test_contract_uses_ready_pair_v2_and_scanner_v1():
    ...
    assert contract.DATASET_ID.endswith("challenger-orb-retest-tranche3-v2")
    assert "tranche3-v2" in str(contract.SOURCE_MANIFEST)
    assert "tranche3-v1" in str(contract.SCANNER_MANIFEST)
    assert contract.DEFAULT_SELECTION_DOC.name == "source-contract-boundary-v2.md"
