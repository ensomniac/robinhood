"""Independently inspect the second-tranche trigger runner and result."""
...
import challenger_orb_retest_preentry_inspection2 as preentry_inspection2
import challenger_orb_retest_trigger_runner2 as challenger
from historical_store import HistoricalStoreError
from learning_data import LearningDataError


class ChallengerTriggerRunnerInspection2Error(RuntimeError):
    """The second-tranche runner contract or result does not rebuild."""
...
def inspect_contract(
    *, manifest_path: Path, env_path: Path, status_path: Path
    ...
    preentry = challenger.preentry2.base
    ...
    if preentry._trigger_path(config.root).exists():
        raise ChallengerTriggerRunnerInspection2Error(
            "second-tranche trigger artifact exists before runner inspection"
        )
    ...
    if not (
        ...
        raise ChallengerTriggerRunnerInspection2Error(
            "second-tranche trigger compatibility contract differs"
        )
    result = {
        "schema_version": 1,
...
def inspect_result(
    *, manifest_path: Path, env_path: Path, status_path: Path, result_path: Path
    ...
    if not (
        ...
        raise ChallengerTriggerRunnerInspection2Error(
            "uninspected second-tranche trigger status differs"
        )
    with runner.using_exact_reader():
        result = preentry_inspection2.inspect_result(
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--result", type=Path, default=challenger.preentry2.DEFAULT_PUBLIC_RESULT
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        ChallengerTriggerRunnerInspection2Error,
        challenger.ChallengerTriggerRunner2Error,
        challenger.base.ChallengerTriggerRunnerError,
        challenger.preentry2.ChallengerPreentry2Error,
        challenger.preentry2.base.ChallengerPreentryError,
