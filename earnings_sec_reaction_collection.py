"""Collect only the 109-symbol graph bound to the inspected v10 search."""
...
PRIVATE_NAMESPACE = "_derived/earnings_sec_reaction_v10"


class EarningsSecReactionCollectionError(RuntimeError):
    """The inspected search or its exact collection graph drifted."""
...
def _fetch(
    ...
    try:
        return yahoo._fetch(request, session, telemetry)
        ...
        value = {
            ...
            "missing_reason": "Yahoo identity or quote envelope unavailable",
            "rows": [],
        }
...
def collect(
    ...
    *,
    ...
    if not (
        ...
        raise EarningsSecReactionCollectionError(
            "v10 search collection authorization is invalid"
        )
    ...
    if canonical_sha256(selected) != contract["universe"]["selection_sha256"]:
        raise EarningsSecReactionCollectionError(
            "v10 metadata selection drifted"
        )
    telemetry: dict[str, Any] = {
        "requests": 0,
        "request_seconds": 0.0,
    ...
    try:
        for index, request in enumerate(
            ...
            if path.is_file():
                task = metadata._read_private(path)
                if not (
                ...
                if (
                    ...
                    telemetry["pacing_wait_seconds"] += yahoo.PACE_SECONDS
                task = (
                    fetcher(request)
                ...
                if not (
                    ...
                    raise EarningsSecReactionCollectionError(
                        "v10 task does not match the frozen request"
                    )
                metadata._write_private(path, task)
            tasks.append(task)
    finally:
        http.close()
    if (
        telemetry["requests"] + telemetry["cache_hits"]
        != len(contract["development_data_requests"])
    ):
        raise EarningsSecReactionCollectionError(
            "v10 request and cache accounting is incomplete"
        )
    ...
    metadata._write_private(private_path, dataset)
    value: dict[str, Any] = {
        "schema_version": 1,
        "artifact_kind": "earnings-sec-reaction-development-collection",
        "campaign_id": CAMPAIGN_ID,
        "family_id": FAMILY_ID,
        "successor_id": SUCCESSOR_ID,
        "state": "REACTION_DEVELOPMENT_COLLECTED_UNINSPECTED",
        "collected_at": metadata._timestamp(collected_at, "collected_at"),
        ...
        "symbols_requested": len(contract["development_data_requests"]),
    ...
    outcome_exposure.ensure_record(
        outcome_exposure.build_record(
            exposure_id=(
                f"source-{FAMILY_ID}-v10-"
                f"{value['collection_sha256'][:16]}"
            ),
            campaign_id=CAMPAIGN_ID,
            lane="development",
            recorded_at=value["collected_at"],
            source_path=metadata._repo_path(path),
        ...
        json.dumps(
            {
                ...
                "symbols_missing": value["symbols_permanently_missing"],
            },
            indent=2,
            sort_keys=True,
        )
