...
def _validated_dataset_id(dataset_id: str) -> str:
    if (
        ...
        raise DevelopmentSecSupplementalCollectionError("dataset ID is unsafe")
    return dataset_id
...
def _response_root(
    ...
    return supplemental_contract._target_response_root(
...
def _wrapper_path(
    ...
    return _response_root(store_root, source_dataset_id) / f"{digest}.json.gz"
...
def _load_contract(
    *,
    ...
    source_manifest_binding = lineage["sec_source_manifest"]
    source_manifest = load_frozen_dataset_contract(
        _resolve_public_path(source_manifest_binding.get("path"))
    )
    if source_manifest.get("dataset_id") != source_dataset_id:
        raise DevelopmentSecSupplementalCollectionError("unexpected SEC source dataset")
    ...
    expected_private_path = (
        ...
        f"{supplemental_contract.PRIVATE_NAMESPACE}/{source_dataset_id}/"
        f"supplemental_contracts/{dataset_id}/"
        f"{supplemental_contract.PRIVATE_CONTRACT_FILE}"
    )
    if selection.get("private_contract_path") != expected_private_path:
        raise DevelopmentSecSupplementalCollectionError(
            "supplemental private namespace drifted"
        )
    private = _read_gzip_object(
        supplemental_contract._private_contract_path(
...
def collect(
    *,
    dataset_id: str = supplemental_contract.DATASET_ID,
    source_dataset_id: str = source_contract.DATASET_ID,
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--dataset-id", default=supplemental_contract.DATASET_ID)
    ...
    inspect_parser.add_argument(
        "--output", type=Path, default=DEFAULT_PUBLIC_INSPECTION
    )
    return parser
