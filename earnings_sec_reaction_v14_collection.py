"""Collect the development-only Yahoo graph bound to the v14 search."""
...
import earnings_sec_reaction_v13_collection as v13_collection
...
PRIVATE_NAMESPACE = "_derived/earnings_sec_reaction_v14"
INVALID_OHLCV_ERROR = "Yahoo chart contains invalid OHLCV"
INVALID_OHLCV_REASON = (
    "Yahoo invalid OHLCV retained as whole-symbol permanent missing"
)


class EarningsSecReactionV14CollectionError(RuntimeError):
    """The inspected v14 search or development graph drifted."""
...
def _fetch(
    ...
    try:
        return v13_collection._fetch(request, session, telemetry)
    except yahoo.EarningsSecYahooDataError as exc:
        if str(exc) != INVALID_OHLCV_ERROR:
            raise
        ...
        value: dict[str, Any] = {
            ...
            "missing_reason": INVALID_OHLCV_REASON,
            "rows": [],
        }
        value["task_sha256"] = capacity.self_hash(
            value, "task_sha256"
        )
        return value
...
def collect(
    ...
    *,
    ...
    if not (
        ...
        and inspection.get("excluded_v13_symbols")
        == search_source.EXPECTED_EXCLUDED_SYMBOLS
        and inspection.get("confirmation_provider_access_authorized")
        is False
    ):
        raise EarningsSecReactionV14CollectionError(
            "v14 search collection authorization is invalid"
        )
    contract = search["family_contract"]
    if (
        contract["development_data_policy"].get("invalid_ohlcv")
        != "whole_symbol_permanent_missing_zero_credit"
    ):
        raise EarningsSecReactionV14CollectionError(
            "v14 invalid-OHLCV policy is not frozen"
        )
    ...
    if (
        ...
        raise EarningsSecReactionV14CollectionError(
            "v14 metadata selection drifted"
        )
    telemetry: dict[str, Any] = {
        "requests": 0,
        "request_seconds": 0.0,
        ...
        for index, request in enumerate(
            ...
            if path.is_file():
                ...
                if not (
                    ...
                    raise EarningsSecReactionV14CollectionError(
                        "v14 task does not match the frozen request"
                    )
    ...
    if telemetry["requests"] + telemetry["cache_hits"] != expected_requests:
        raise EarningsSecReactionV14CollectionError(
            "v14 request and cache accounting is incomplete"
        )
    ...
    dataset = {
        ...
        "source_semantics": {
            ...
            "invalid_ohlcv_is_whole_symbol_permanent_missing": True,
            "v13_tasks_reused": False,
    ...
    value: dict[str, Any] = {
        "schema_version": 1,
        "artifact_kind": (
            "earnings-sec-reaction-v14-development-collection"
        ),
        "campaign_id": CAMPAIGN_ID,
        "family_id": FAMILY_ID,
        "successor_id": SUCCESSOR_ID,
        "state": "REACTION_V14_DEVELOPMENT_COLLECTED_UNINSPECTED",
        ...
        "invalid_ohlcv_symbols": sorted(
            ...
            if reason == INVALID_OHLCV_REASON
        ),
