"""Independently inspect challenger pre-entry contracts and trigger results."""
...
class ChallengerPreentryInspectionError(RuntimeError):
    """The independently reconstructed pre-entry evidence differs."""
...
def inspect_contract(*, manifest_path: Path, env_path: Path) -> dict:
    ...
    rebuilt = preentry.build_selection(config.root)
    contract = manifest.get("selection_contract", {})
    if not (
        manifest.get("dataset_id") == preentry.DATASET_ID
        ...
        and rebuilt["counts"]["verified_positive_pairs"] == 95
        and rebuilt["counts"]["verified_positive_dates"] == 47
        and rebuilt["counts"]["total_requests"] == 284
    ):
        raise ChallengerPreentryInspectionError("pre-entry selection does not rebuild")
    for value in manifest.get("implementation_contract", {}).values():
        path = preentry.PROJECT_ROOT / str(value["path"])
        if preentry._sha256_file(path) != value.get("sha256"):
            raise ChallengerPreentryInspectionError("implementation binding differs")
    root = preentry._private_root(config.root)
    artifacts = {
        "frozen_selections": int(preentry._selection_path(config.root).is_file()),
        "terminal_wrappers": len(list((root / "wrappers").glob("*.json.gz")))
        if (root / "wrappers").exists() else 0,
    ...
    if artifacts != {
        ...
        raise ChallengerPreentryInspectionError("pre-entry zero state differs")
    if shutil.disk_usage(config.root).free < config.min_free_bytes:
        raise ChallengerPreentryInspectionError("historical reserve is unavailable")
    return {
        "schema_version": 1,
...
def inspect_result(
    *, manifest_path: Path, env_path: Path, result_path: Path
    ...
    if rebuilt != recorded:
        raise ChallengerPreentryInspectionError("pre-entry trigger result differs")
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--env-file", type=Path, default=preentry.PROJECT_ROOT / ".env")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "inspect-contract":
            value = inspect_contract(manifest_path=args.manifest, env_path=args.env_file)
            preentry._write_json(args.status, value)
        else:
        ...
        ChallengerPreentryInspectionError,
