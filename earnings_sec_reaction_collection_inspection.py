"""Independently inspect v10 data and bind it to the frozen search."""
...
import earnings_sec_reaction_collection as source
...
class EarningsSecReactionCollectionInspectionError(RuntimeError):
    """The exact v10 dataset failed independent validation."""
...
def inspect_collection(
    ...
    *,
    ...
    inspection_path = metadata.PROJECT_ROOT / collection["inspection_path"]
    ...
    if (
        ...
        raise EarningsSecReactionCollectionInspectionError(
            "private v10 dataset is missing or drifted"
        )
    try:
        dataset = json.loads(gzip.decompress(private_path.read_bytes()))
    except (OSError, json.JSONDecodeError) as exc:
        raise EarningsSecReactionCollectionInspectionError(
            "private v10 dataset is unreadable"
        ) from exc
    ...
    if not all(checks.values()):
        raise EarningsSecReactionCollectionInspectionError(
            "v10 development collection inspection failed"
        )
    observed = metadata._timestamp(inspected_at, "inspected_at")
    manifest_path, manifest = freeze_dataset_contract(
        {
            "schema_version": 1,
            "dataset_id": f"dataset-{search_source.SUCCESSOR_ID}-development",
    ...
    value: dict[str, Any] = {
        "schema_version": 1,
        "artifact_kind": "earnings-sec-reaction-collection-inspection",
        "campaign_id": source.CAMPAIGN_ID,
        "family_id": source.FAMILY_ID,
        "successor_id": source.SUCCESSOR_ID,
        "state": "REACTION_DEVELOPMENT_DATASET_INSPECTED_READY",
