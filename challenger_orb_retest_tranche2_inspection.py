"""Independently rebuild the challenger's second frozen date selection."""
...
class ChallengerTranche2InspectionError(RuntimeError):
    """Independent second-tranche reconstruction found a mismatch."""


def inspect(*, manifest_path: Path, env_path: Path) -> dict[str, Any]:
    manifest = load_frozen_dataset_contract(manifest_path)
    if manifest.get("dataset_id") != tranche.DATASET_ID:
        raise ChallengerTranche2InspectionError("selection dataset identity differs")
    ...
    if tranche._sha256_file(private_path) != contract.get("private_exclusion_sha256"):
        raise ChallengerTranche2InspectionError("private exclusions changed")
    if tranche._sha256_file(tranche.DEFAULT_SELECTION) != contract.get(
        "public_selection_sha256"
    ):
        raise ChallengerTranche2InspectionError("public selection changed")
    ...
    comparisons = {
        ...
        "inspected entities": (
            exclusions["registered_inspected_evidence"]["inspected_entity_set_sha256"],
            current["registered_inspected_evidence"]["inspected_entity_set_sha256"],
        ),
        "inspected artifacts": (
    ...
    for label, (frozen, observed) in comparisons.items():
        if frozen != observed:
            raise ChallengerTranche2InspectionError(f"{label} changed after freeze")
    ...
    if rebuilt != selection:
        raise ChallengerTranche2InspectionError("date selection does not rebuild")
    if set(selection["selected_dates"]) & set(exclusions["excluded_dates"]):
        raise ChallengerTranche2InspectionError("selection overlaps excluded dates")
    if sorted(selection["selected_dates"]) != manifest.get("requested_dates"):
        raise ChallengerTranche2InspectionError("manifest date set differs")
    result = {
        "schema_version": 1,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        ChallengerTranche2InspectionError,
        tranche.ChallengerTranche2Error,
