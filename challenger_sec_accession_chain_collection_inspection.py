"""Independently inspect challenger accession transport before semantics."""
...
import challenger_sec_accession_chain_recovery as challenger
...
DEFAULT_RESULT = (
    PROJECT_ROOT
    / "research_results/2026-07-21-challenger-sec-accession-chain-collection-inspection.json"
)


class ChallengerSecAccessionCollectionInspectionError(RuntimeError):
    """The frozen recovery collection does not independently reconcile."""
...
def inspect_collection(
    *, manifest_path: Path, env_path: Path, result_path: Path
    ...
    if rebuilt_index != index:
        raise ChallengerSecAccessionCollectionInspectionError(
    ...
    for request_sha256, request in sorted(expected.items()):
        ...
        wrapper_hashes.append(
            {"request_sha256": request_sha256, "wrapper_sha256": record["wrapper_sha256"]}
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        ChallengerSecAccessionCollectionInspectionError,
        challenger.ChallengerSecAccessionChainError,
