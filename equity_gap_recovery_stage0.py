"""Freeze and evaluate the preregistered equity gap-recovery Stage 0 trial."""
...
SIGNAL_END = time(12, 0)
FORCE_FLAT = time(15, 50)
OPENING_RANGE_BARS = 15
NO_NEW_LOW_BARS = 10


class EquityGapRecoveryError(RuntimeError):
    """The equity gap-recovery Stage 0 evidence is invalid."""
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise EquityGapRecoveryError(str(exc)) from exc
    return value
...
def _variant() -> dict[str, Any]:
    ...
    if len(matches) != 1:
        raise EquityGapRecoveryError("frozen slate variant identity is unavailable")
    ...
    manifest.update(
        {
            ...
            "selection_contract": {
                ...
                "gap_gate": "09:30 open divided by prior completed regular-session close minus one is within -0.08 through -0.02 inclusive",
                "session_open": "09:30 one-minute bar open",
                ...
                "stabilization": "the decision-time session low was established before the ten completed one-minute bars ending with the signal bar; equal lows do not reset the clock",
                "trigger": "completed close strictly above both session open and session VWAP",
                "signal_bar_starts_et": "09:45:00-12:00:00 inclusive",
                "entry": "next observed one-minute open",
                "daily_selection": "earliest entry, then most negative gap, then lexical symbol",
                "miss_policy": "missing prior close, invalid bars, nonpositive stop distance, or nonpositive target distance remains in denominator and cannot signal",
            },
            "outcome_contract": {
                "stop": "lowest completed session low through the signal bar",
                "target": "prior close or raw 2R above entry, whichever is nearer; target must remain above entry",
...
def _validate_manifest(recorded: Mapping[str, Any]) -> None:
    if recorded.get("manifest_sha256") != common._self_hash(
        ...
        raise EquityGapRecoveryError("activation manifest content hash is invalid")
    if (
        ...
        raise EquityGapRecoveryError("activation manifest identity is invalid")
    if recorded.get("implementation_sha256") != sha256_file(Path(__file__).resolve()):
        raise EquityGapRecoveryError("activation implementation drifted")
    for field in (
        "development_evidence_eligible",
        "confirmation_evidence_eligible",
        "provider_requests_authorized",
        ...
        if recorded.get(field) is not False:
            raise EquityGapRecoveryError(f"activation manifest {field} must be false")
...
def inspect_activation(
    ...
    if recorded != build_manifest(source):
        raise EquityGapRecoveryError("activation manifest does not exactly rebuild")
...
def _validate_inspection(
    ...
    if recorded.get("inspection_sha256") != common._self_hash(
        ...
        raise EquityGapRecoveryError("activation inspection content hash is invalid")
    if recorded != inspect_activation(manifest_path, store):
        raise EquityGapRecoveryError("activation inspection does not exactly rebuild")
...
def _gap_in_range(gap: float) -> bool:
    return -0.08 - 1e-12 <= gap <= -0.02 + 1e-12
...
def _candidate(
    *, day: str, raw: Mapping[str, Any], prior_close: float | None
    ...
    if not isinstance(bars, list):
        raise EquityGapRecoveryError(f"{day} {symbol}: bars are malformed")
    ...
    gap_fraction = session_open / prior_close - 1
    ...
    for index in range(OPENING_RANGE_BARS, len(bars) - 1):
        ...
        if observed > SIGNAL_END:
            break
        stable_boundary = index - NO_NEW_LOW_BARS + 1
        earlier_low = min(lows[:stable_boundary])
        recent_low = min(lows[stable_boundary : index + 1])
        if recent_low < earlier_low:
            continue
        if (
            float(bars[index]["close"]) <= session_open
            or float(bars[index]["close"]) <= vwap[index]
        ):
            continue
        entry_index = index + 1
        entry_open = float(bars[entry_index]["open"])
        stop = min(lows[: index + 1])
        if entry_open <= stop:
            return {
                "date": day,
                "symbol": symbol,
        ...
        target = min(prior_close, entry_open + 2 * (entry_open - stop))
    ...
    return {"date": day, "symbol": symbol, "status": "no_recovery_trigger"}
...
def _trade_outcome(
    ...
    if exit_price is None:
        raise EquityGapRecoveryError("trade did not resolve by force-flat")
    ...
    if planned_risk <= 0:
        raise EquityGapRecoveryError("cost-adjusted planned risk is not positive")
    return {
...
def build_result(
    ...
    *,
    ...
    for session in sessions:
        ...
        candidates = [
            _candidate(day=day, raw=item["raw"], prior_close=item["prior_close"])
...
def inspect_result(
    ...
    if recorded.get("result_sha256") != common._self_hash(recorded, "result_sha256"):
        raise EquityGapRecoveryError("Stage 0 result content hash is invalid")
    ...
    if recorded != rebuilt:
        raise EquityGapRecoveryError("Stage 0 result does not independently rebuild")
    audit: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
...
def _publish(value: Mapping[str, Any], path: Path) -> None:
    ...
    if not identity or not path.name.endswith(f"-{identity}.json"):
        raise EquityGapRecoveryError("output filename must end with its content hash")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except (EquityGapRecoveryError, continuation.EquityGapStage0Error, OSError) as exc:
